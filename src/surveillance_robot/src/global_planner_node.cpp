...
{ 
    ...
    for (int count = 0; count < V - 1; count++) { 
        ...
        // yet processed. u is always equal to currentposition in the first iteration. 
...
int main(int argc, char **argv){
    ...
    if (current_position_estim.y<p3.y && current_position_estim.y>p2 && goal_to_reach.x<p4 && goal_to_reach.x>p2 ||
        goal_to_reach.y<p3.y && goal_to_reach.y>p2 && current_position_estim.x<p4 && current_position_estim.x>p2 )
    {   
        float d3c = distancePoints (p3, current_position_estim);
        float d2c = distancePoints (p2, current_position_estim);
        float d2g = distancePoints (p2, goal_to_reach);
        float d4g = distancePoints (p4, goal_to_reach);

        float graph[6][6] = {   { 0, d12, 0, 0, 0, 0 }, 
                                { d12, 0, 0, 0, d2c, d2g }, 
                                { 0, 0, 0, 0, d3c, 0 }, 
                                { 0, 0, 0, 0, 0, d4g }, 
                                { 0, d2c, d3c, 0, 0, 0 }, 
    ...
    if (current_position_estim.y<p2.y && current_position_estim.y>p1 && goal_to_reach.x<p4 && goal_to_reach.x>p2 ||
        goal_to_reach.y<p2.y && goal_to_reach.y>p1 && current_position_estim.x<p4 && current_position_estim.x>p2)
    {   
        float d1c = distancePoints (p1, current_position_estim);
        float d2c = distancePoints (p2, current_position_estim);
        float d2g = distancePoints (p2, goal_to_reach);
        float d4g = distancePoints (p4, goal_to_reach);

        float graph[6][6] = { { 0, 0, 0, 0, d1c, 0 }, 
                            { 0, 0, d23, 0, d2c, d2g }, 
                            { 0, d23, 0, 0, 0, 0 }, 
                            { 0, 0, 0, 0, 0, d4g }, 
                            { d1c, d2c, 0, 0, 0, 0 }, 
